...
| **Included / IncludedCollection** | The JSON:API `included` array in a response. In this library it is typed as `IncludedCollection?` on `IDataResult`. `IncludedCollection` stores the raw `JsonElement` items and exposes `OfType<T>() : IEnumerable<Data<T>>` and `FindById<T>(string id) : Data<T>?` — both use `PingenSerialisationHelper.PingenApiDataTypeMapping` to map the JSON `type` string to a CLR attributes type. The earlier convenience `PingenSerialisationHelper.TryGetIncludedData<T>(IDataResult, out Data<T>?)` is still available and simply delegates to `OfType<T>().SingleOrDefault()`. |
