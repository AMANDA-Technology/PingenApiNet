...
| **`PingenApiDataTypeMapping` coverage** | `src/PingenApiNet.Abstractions/Helpers/PingenSerialisationHelper.cs:116-132` | `IncludedCollection.OfType<T>()`, `FindById<T>()`, and `TryGetIncludedData<T>()` all rely on this dictionary. Adding a new resource type (new `PingenApiDataType` enum value + new attributes type) requires a corresponding mapping entry, otherwise included resources of that type are silently skipped with no warning. | Every new resource type **must** be added to `PingenApiDataTypeMapping`. This is now enforced by `PingenApiDataTypeMappingTests` which fails if a new enum value lacks a mapping or an explicit allow-list entry. **The reverse hazard exists too:** `TryGetIncludedData<T>()` is `OfType<T>().SingleOrDefault()`, so mapping two discriminators to one attributes type throws as soon as Pingen includes both. `letters_events` and `deliverables_events` are exactly that pair since 2026-07-27, and both *must* stay mapped — whichever one you drop, event resolution silently returns `null` on the day Pingen stops sending the other. The duplicate is handled in `IncludedCollection.OfType<T>()`, which collapses one resource appearing under two different discriminators with a shared `id`; that collapse is load-bearing, not a tidy-up. It is deliberately narrow — repeats under the *same* discriminator still come back in full. |
| **Unknown top-level `data.type` is fatal** | `src/PingenApiNet.Abstractions/Models/Base/DataIdentity.cs:41` + `PingenApiDataType.cs` | `DataIdentity.Type` is a non-nullable `PingenApiDataType` bound by a strict `JsonStringEnumConverter`. An unknown `included[].type` is tolerated (`IncludedCollection` does `Enum.TryParse` and skips), but an unknown **top-level** `type` throws `JsonException` and makes the whole body unparseable. The same is true of every *relationship* discriminator (`RelatedSingleOutput.Data.Type` is the same non-nullable enum) — that is a second, less obvious blast radius. For webhooks either one throws *after* the HMAC signature has validated, so the consumer 5xx's and Pingen dead-letters the event (the `webhook_delivered` outage came from the top-level `type`, the `deliverables_events` outage of 2026-07-27 from `data.relationships.event.data.type` — see `doc/analysis/2026-05-01-api-docs-gap-audit.md` §§ addenda). | A new Pingen `type` discriminator must be added to `PingenApiDataType` **and** bound to an attributes model that actually fits it. Never enumerate a type just to stop the throw: the body then binds to a mismatched model, silently dropping attributes and writing `null` into non-nullable relationship properties. That is why `channel_subscriptions` is deliberately **not** enumerated. |
| **`letter` vs `deliverable` relationships** | `src/PingenApiNet.Abstractions/Models/Webhooks/WebhookEvents/WebhookEventRelationships.cs`, `src/PingenApiNet.Abstractions/Models/LetterEvents/LetterEventRelationships.cs` | Pingen's 2026-07-27 rollout replaced the `letter` relationship with `deliverable` (`letters` \| `emails` \| `ebills`) on every webhook body. The spec no longer declares `letter` at all — `WebhookDeliverable*GET` requires `organisation` + `deliverable` + `event` — but the wire still sends it. Both are modelled and **both are nullable**, so exactly one of them is null depending on which side of the migration a given body came from. | Read `Deliverable` first and fall back to `Letter`; never dereference either unconditionally. Do not "clean up" the nullability: making `Letter` non-nullable again re-arms the NRE for the day Pingen finishes the migration, and `RespectNullableAnnotations` is unset so the compiler will not catch it. `ValidateWebhookAndGetData_LegacyLetterShapeDropped_StillResolvesEverything` pins the post-migration shape. |
| **Enum ordinals are public ABI** | `src/PingenApiNet.Abstractions/Enums/Api/PingenApiDataType.cs`, `WebhookEventCategory.cs` | C# inlines enum constants into consumer IL. Inserting a member mid-enum renumbers every later member, so a consumer assembly compiled against an older package (or a value persisted as an `int`) silently reinterprets it — no exception, since the stale int lands on a valid member. The wire format is unaffected (`JsonStringEnumConverter` serializes by name), which is exactly what makes this invisible in tests. | Both enums carry explicit numeric values. **Append only** — take the next free number; never insert or reorder. `AllEnumsSerializationTests.PingenApiDataType_OrdinalsAreStable` / `WebhookEventCategory_OrdinalsAreStable` fail if an existing member is renumbered. |
