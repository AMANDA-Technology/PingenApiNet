...
| **Unknown top-level `data.type` is fatal** | `src/PingenApiNet.Abstractions/Models/Base/DataIdentity.cs:41` + `PingenApiDataType.cs` | `DataIdentity.Type` is a non-nullable `PingenApiDataType` bound by a strict `JsonStringEnumConverter`. An unknown `included[].type` is tolerated (`IncludedCollection` does `Enum.TryParse` and skips), but an unknown **top-level** `type` throws `JsonException` and makes the whole body unparseable. For webhooks this throws *after* the HMAC signature has validated, so the consumer 5xx's and Pingen dead-letters the event (this is the `webhook_delivered` outage — see `doc/analysis/2026-05-01-api-docs-gap-audit.md` § addendum). | A new Pingen `type` discriminator must be added to `PingenApiDataType` **and** bound to an attributes model that actually fits it. Never enumerate a type just to stop the throw: the body then binds to a mismatched model, silently dropping attributes and writing `null` into non-nullable relationship properties. That is why `channel_subscriptions` is deliberately **not** enumerated. |
